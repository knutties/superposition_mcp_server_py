...
async def test_get_context(env: None) -> None:
    ...
    with patch("superposition_mcp.tools.context.get_client", AsyncMock(return_value=client)):
        result = await get_context(org_id="o1", workspace_id="prod", id="c1", ctx=make_stdio_ctx())
    assert result["id"] == "c1"
...
async def test_get_context_from_condition(env: None) -> None:
    ...
    with patch("superposition_mcp.tools.context.get_client", AsyncMock(return_value=client)):
        result = await get_context_from_condition(
            org_id="o1", workspace_id="prod", context=cond, ctx=make_stdio_ctx()
        )
    assert result["id"] == "c2"
...
async def test_list_contexts(env: None) -> None:
    ...
    with patch("superposition_mcp.tools.context.get_client", AsyncMock(return_value=client)):
        await list_contexts(
            org_id="o1", workspace_id="prod", ctx=make_stdio_ctx(), count=20, page=1, prefix="foo."
        )
